...
def set_oscillator(myBackend, freq_hz):
	osc=myBackend.getLMK61E2()[0]
	osc.getFrequency()
	osc.setFrequency(freq_hz)
...
set_oscillator(myBackend, 416e6)

		
ValidNumTDL = 0              # Change this value if you want to select another TDL to extract the valid
ValidPositionTap = 1         # Change this value if you want to select another tap (of ValidPosition_SampledTaps, see VHDL) to extract the valid
...
histo.setMeasCH(2, tdcModule.RISING, False)
histo.setRefCH(1, tdcModule.RISING, False)
